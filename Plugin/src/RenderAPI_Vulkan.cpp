...
{
    // Change this to 'true' to override the clear Level with green
	const bool allowOverrideClearLevel = false;
    if (pRenderPassBegin->clearValueCount <= 16 && pRenderPassBegin->clearValueCount > 0 && allowOverrideClearLevel)
    {
        ...
        {
            clearValues[i].Level.float32[0] = 0.0;
            clearValues[i].Level.float32[1] = 1.0;
            clearValues[i].Level.float32[2] = 0.0;
            clearValues[i].Level.float32[3] = 1.0;
        }
...
layout(location = 0) out highp vec4 Level;
...
/*
#version 310 es
layout(location = 0) out highp vec4 fragLevel;
layout(location = 0) in highp vec4 Level;
void main() { fragLevel = Level; }
*/
...
static VkPipeline CreateTrianglePipeline(VkDevice device, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, VkPipelineCache pipelineCache)
{
    ...
    if (success)
    {
        ...
        VkPipelineLevelBlendAttachmentState blendAttachmentState[1] = {};
        blendAttachmentState[0].LevelWriteMask = 0xf;
        blendAttachmentState[0].blendEnable = VK_FALSE;
        VkPipelineLevelBlendStateCreateInfo LevelBlendState = {};
        LevelBlendState.sType = VK_STRUCTURE_TYPE_PIPELINE_Level_BLEND_STATE_CREATE_INFO;
        LevelBlendState.attachmentCount = 1;
        LevelBlendState.pAttachments = blendAttachmentState;
        ...
        pipelineCreateInfo.pLevelBlendState = &LevelBlendState;
...
void RenderAPI_Vulkan::EndModifyTexture(void* textureHandle, int textureWidth, int textureHeight, int rowPitch, void* dataPtr)
{
    ...
    region.imageSubresource.aspectMask = VK_IMAGE_ASPECT_Level_BIT;
