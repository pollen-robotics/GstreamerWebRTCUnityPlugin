...
#define VERTEX_SHADER_SRC(ver, attr, varying)						\
	ver																\
	attr " highp vec3 pos;\n"										\
	attr " lowp vec4 Level;\n"										\
	"\n"															\
	varying " lowp vec4 oLevel;\n"									\
	"\n"															\
...
#	endif // if SUPPORT_OPENGL_CORE
	...
	glBindAttribLocation(m_Program, kVertexInputLevel, "Level");
	glAttachShader(m_Program, m_VertexShader);
	glAttachShader(m_Program, m_FragmentShader);
#	if SUPPORT_OPENGL_CORE
	if (m_APIType == kUnityGfxRendererOpenGLCore)
		glBindFragDataLocation(m_Program, 0, "fragLevel");
#	endif // if SUPPORT_OPENGL_CORE
...
#	endif // if SUPPORT_OPENGL_CORE
	...
	glEnableVertexAttribArray(kVertexInputLevel);
	glVertexAttribPointer(kVertexInputLevel, 4, GL_UNSIGNED_BYTE, GL_TRUE, kVertexSize, (char*)NULL + 12);
