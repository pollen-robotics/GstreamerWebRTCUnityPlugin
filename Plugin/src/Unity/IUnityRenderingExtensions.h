...
/*
    ...
    On top of the Low-level native plugin interface, Unity also supports low level rendering extensions that can receive
   callbacks when certain events happen. This is mostly used to implement and control low-level rendering in your plugin and
   enable it to work with Unity’s multithreaded rendering.

    Due to the low-level nature of this extension the plugin might need to be preloaded before the devices get created.
    Currently the convention is name-based namely the plugin name must be prefixed by “GfxPlugin”. Example:
   GfxPluginMyFancyNativePlugin.
...
//     These events will be propagated to all plugins that implement void UnityRenderingExtEvent(UnityRenderingExtEventType
//     event, void* data);
...
typedef enum UnityRenderingExtEventType
{
    ...
    kUnityRenderingExtEventSetStereoEye,    // issued during stereo rendering at the beginning of each eye's rendering loop. It
                                            // carries the current 'eye' index as parameter
    kUnityRenderingExtEventStereoRenderingDone, // issued after the rendering has finished
    kUnityRenderingExtEventBeforeDrawCall, // issued during BeforeDrawCall and carrying UnityRenderingExtBeforeDrawCallParams as
                                           // parameter
    ...
    kUnityRenderingExtEventCustomGrab,     // issued during GrabIntoRenderTexture since we can't simply copy the resources
                                           //      when custom rendering is used - we need to let plugin handle this. It carries
                                       //      over a UnityRenderingExtCustomBlitParams params = { X, source, dest, 0, 0 } ( X
                                       //      means it's irrelevant )
    kUnityRenderingExtEventCustomBlit,         // issued by plugin to insert custom blits. It carries over
                                               // UnityRenderingExtCustomBlitParams as param.
    ...
    kUnityRenderingExtEventUpdateTextureBeginV1 =
        kUnityRenderingExtEventUpdateTextureBegin, // Deprecated. Issued to update a texture. It carries over
                                                   // UnityRenderingExtTextureUpdateParamsV1
    kUnityRenderingExtEventUpdateTextureEndV1 =
        kUnityRenderingExtEventUpdateTextureEnd, // Deprecated. Issued to signal the plugin that the texture update has
                                                 // finished. It carries over the same UnityRenderingExtTextureUpdateParamsV1 as
                                                 // kUnityRenderingExtEventUpdateTextureBeginV1
    kUnityRenderingExtEventUpdateTextureBeginV2, // Issued to update a texture. It carries over
                                                 // UnityRenderingExtTextureUpdateParamsV2
    kUnityRenderingExtEventUpdateTextureEndV2,   // Issued to signal the plugin that the texture update has finished. It carries
                                                 // over the same UnityRenderingExtTextureUpdateParamsV2 as
                                                 // kUnityRenderingExtEventUpdateTextureBeginV2
...
{
    kUnityRenderingExtCustomBlitVRFlush, // This event is mostly used in multi GPU configurations ( SLI, etc ) in order to allow
                                         // the plugin to flush all GPU's targets
...
{
    kUnityRenderingExtQueryOverrideViewport =
        1 << 0, // The plugin handles setting up the viewport rects. Unity will skip its internal SetViewport calls
    kUnityRenderingExtQueryOverrideScissor =
        1 << 1, // The plugin handles setting up the scissor rects. Unity will skip its internal SetScissor calls
    kUnityRenderingExtQueryOverrideVROcclussionMesh = 1 << 2, // The plugin handles its own VR occlusion mesh rendering. Unity
                                                              // will skip rendering its internal VR occlusion mask
    kUnityRenderingExtQueryOverrideVRSinglePass =
        1 << 3, // The plugin uses its own single pass stereo technique. Unity will only traverse and render the render node
                // graph once.
    ...
    kUnityRenderingExtQueryKeepOriginalDoubleWideWidth_DEPRECATED =
        1 << 4, // Instructs unity to keep the original double wide width. By default unity will try and have a power-of-two
                // width for mip-mapping requirements.
    kUnityRenderingExtQueryRequestVRFlushCallback = 1 << 5, // Instructs unity to provide callbacks when the VR eye textures
                                                            // need flushing. Useful for multi GPU synchronization.
    kUnityRenderingExtQueryOverridePresentFrame =
        1 << 6, // The plugin handles it's own SwapChain Present. Unity will skip its internal Present calls
...
typedef struct UnityRenderingExtCustomBlitParams
{
    ...
    unsigned int
        command; // command for the custom blit - could be any UnityRenderingExtCustomBlitCommands command or custom ones.
