...
UNITY_DECLARE_INTERFACE(IUnityGraphics)
{
    ...
    int(UNITY_INTERFACE_API *
        ReserveEventIDRange)(int count); // reserves 'count' event IDs. Plugins should use the result as a base index when
                                         // issuing events back and forth to avoid event id clashes.
};
